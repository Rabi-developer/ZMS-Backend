...
public interface IExpenseService : IBaseService<ExpenseReq, ExpenseRes, Expense>
{
    Task<IList<Expense>> GetByParent(Guid ParentId);
}
public class ExpenseService : BaseService<ExpenseReq, ExpenseRes, ExpenseRepository, Expense>, IExpenseService
{
    ...
    public ExpenseService(IUnitOfWork unitOfWork, ApplicationDbContext dbContext) : base(unitOfWork)
    {
        _context = dbContext;
        UnitOfWork = unitOfWork;
        _repository = UnitOfWork.GetRepository<ExpenseRepository>();

    }



    public async Task<IList<Expense>> GetByParent(Guid ParentId)
    {
        return await Repository.GetByParent(ParentId);
    }

    public async override Task<Response<Guid>> Add(ExpenseReq reqModel)
    {
        try
        {
            // Fetch the parent account if ParentAccountId is provided
            Expense parentAccount = null;
            if (reqModel.ParentAccountId.HasValue)
            {
                parentAccount = await _context.Expenses
            ...
            else
            {
                ...
                var siblings = await _context.Expenses
            ...
            var entity = reqModel.Adapt<Expense>();
            entity.Listid = listId;

            // Add the entity to the repository
            var ss = await Repository.Add((Expense)(entity as IMinBase ??
    ...
    public override async Task<Response<IList<ExpenseRes>>> GetAll(Pagination? paginate)
    {
        try
        {
            var pagination = paginate ?? new Pagination();

            // Include ExpenseProducts in the query
           ...
           .OrderBy(x => x.Id)
            ...
            if (!data.Any())
            {
                return new Response<IList<ExpenseRes>>
                {
                    Data = new List<ExpenseRes>(),
                    Misc = pag,
            ...
            var ExpenseResList = data.Adapt<List<ExpenseRes>>();

            //// Manually map ExpenseProducts to Products in the response
            //foreach (var ExpenseRes in ExpenseResList)
            //{
            //    var Expense = data.FirstOrDefault(t => t.Id == ExpenseRes.Id);
            //    if (Expense != null)
            //    {
            //        ExpenseRes.Products = Expense.ExpenseProducts
            //            .Select(tp => new ExpenseProductRes
            //            {
