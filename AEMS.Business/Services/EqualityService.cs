...
public interface IEqualityService : IBaseService<EqualityReq, EqualityRes, Equality>
{
    Task<IList<Equality>> GetByParent(Guid ParentId);
}
public class EqualityService : BaseService<EqualityReq, EqualityRes, EqualityRepository, Equality>, IEqualityService
{
    private readonly ApplicationDbContext _context;
    private readonly IEqualityRepository _repository;
    protected readonly IUnitOfWork UnitOfWork;


    public EqualityService(IUnitOfWork unitOfWork, ApplicationDbContext dbContext) : base(unitOfWork)
    {
        _context = dbContext;
        UnitOfWork = unitOfWork;
        _repository = UnitOfWork.GetRepository<EqualityRepository>();

    }



    public async Task<IList<Equality>> GetByParent(Guid ParentId)
    {
        return await Repository.GetByParent(ParentId);
    }

    public async override Task<Response<Guid>> Add(EqualityReq reqModel)
    {
        try
        {
            // Fetch the parent account if ParentAccountId is provided
            Equality parentAccount = null;
            if (reqModel.ParentAccountId.HasValue)
            {
                parentAccount = await _context.Equality
            ...
            else
            {
                ...
                var siblings = await _context.Equality
            ...
            var entity = reqModel.Adapt<Equality>();
            entity.Listid = listId;

            // Add the entity to the repository
            var ss = await Repository.Add((Equality)(entity as IMinBase ??
    ...
    public override async Task<Response<IList<EqualityRes>>> GetAll(Pagination? paginate)
    {
        try
        {
            var pagination = paginate ?? new Pagination();

            // Include EqualityProducts in the query
           ...
           .OrderBy(x => x.Id)
            ...
            if (!data.Any())
            {
                return new Response<IList<EqualityRes>>
                {
                    Data = new List<EqualityRes>(),
                    Misc = pag,
            ...
            var EqualityResList = data.Adapt<List<EqualityRes>>();

            //// Manually map EqualityProducts to Products in the response
            //foreach (var EqualityRes in EqualityResList)
            //{
            //    var Equality = data.FirstOrDefault(t => t.Id == EqualityRes.Id);
            //    if (Equality != null)
            //    {
            //        EqualityRes.Products = Equality.EqualityProducts
            //            .Select(tp => new EqualityProductRes
            //            {
