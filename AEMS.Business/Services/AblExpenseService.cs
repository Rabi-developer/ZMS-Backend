...
public interface IAblExpenseService : IBaseService<AblExpenseReq, AblExpenseRes, AblExpense>
{
    Task<IList<AblExpense>> GetByParent(Guid ParentId);
}
public class AblExpenseService : BaseService<AblExpenseReq, AblExpenseRes, AblExpenseRepository, AblExpense>, IAblExpenseService
{
    private readonly ApplicationDbContext _context;
    private readonly IAblExpenseRepository _repository;
    protected readonly IUnitOfWork UnitOfWork;


    public AblExpenseService(IUnitOfWork unitOfWork, ApplicationDbContext dbContext) : base(unitOfWork)
    {
        _context = dbContext;
        UnitOfWork = unitOfWork;
        _repository = UnitOfWork.GetRepository<AblExpenseRepository>();

    }



    public async Task<IList<AblExpense>> GetByParent(Guid ParentId)
    {
        return await Repository.GetByParent(ParentId);
    }

    public async override Task<Response<Guid>> Add(AblExpenseReq reqModel)
    {
        try
        {
            // Fetch the parent account if ParentAccountId is provided
            AblExpense parentAccount = null;
            if (reqModel.ParentAccountId.HasValue)
            {
                parentAccount = await _context.AblExpense
            ...
            else
            {
                ...
                var siblings = await _context.AblExpense
            ...
            var entity = reqModel.Adapt<AblExpense>();
            entity.Listid = listId;

            // Add the entity to the repository
            var ss = await Repository.Add((AblExpense)(entity as IMinBase ??
    ...
    public override async Task<Response<IList<AblExpenseRes>>> GetAll(Pagination? paginate)
    {
        try
        {
            var pagination = paginate ?? new Pagination();

            // Include AblExpenseProducts in the query
           ...
           .OrderBy(x => x.Id)
            ...
            if (!data.Any())
            {
                return new Response<IList<AblExpenseRes>>
                {
                    Data = new List<AblExpenseRes>(),
                    Misc = pag,
            ...
            var AblExpenseResList = data.Adapt<List<AblExpenseRes>>();

            //// Manually map AblExpenseProducts to Products in the response
            //foreach (var AblExpenseRes in AblExpenseResList)
            //{
            //    var AblExpense = data.FirstOrDefault(t => t.Id == AblExpenseRes.Id);
            //    if (AblExpense != null)
            //    {
            //        AblExpenseRes.Products = AblExpense.AblExpenseProducts
            //            .Select(tp => new AblExpenseProductRes
            //            {
