...
public interface IAssetsService : IBaseService<AssetsReq, AssetsRes, Assets>
{
    Task<IList<Assets>> GetByParent(Guid ParentId);
}
public class AssetsService : BaseService<AssetsReq, AssetsRes, AssetsRepository, Assets>, IAssetsService
{
    ...
    public AssetsService(IUnitOfWork unitOfWork, ApplicationDbContext dbContext) : base(unitOfWork)
    {
        _context = dbContext;
        UnitOfWork = unitOfWork;
        _repository = UnitOfWork.GetRepository<AssetsRepository>();

    }



    public async Task<IList<Assets>> GetByParent(Guid ParentId)
    {
        return await Repository.GetByParent(ParentId);
    }

    public async override Task<Response<Guid>> Add(AssetsReq reqModel)
    {
        try
        {
            // Fetch the parent account if ParentAccountId is provided
            Assets parentAccount = null;
            if (reqModel.ParentAccountId.HasValue)
            {
                parentAccount = await _context.Assets
            ...
            else
            {
                ...
                var siblings = await _context.Assets
            ...
            var entity = reqModel.Adapt<Assets>();
            entity.Listid = listId;

            // Add the entity to the repository
            var ss = await Repository.Add((Assets)(entity as IMinBase ??
    ...
    public override async Task<Response<IList<AssetsRes>>> GetAll(Pagination? paginate)
    {
        try
        {
            var pagination = paginate ?? new Pagination();

            // Include AssetsProducts in the query
           ...
           .OrderBy(x => x.Id)
            ...
            if (!data.Any())
            {
                return new Response<IList<AssetsRes>>
                {
                    Data = new List<AssetsRes>(),
                    Misc = pag,
            ...
            var AssetsResList = data.Adapt<List<AssetsRes>>();

            //// Manually map AssetsProducts to Products in the response
            //foreach (var AssetsRes in AssetsResList)
            //{
            //    var Assets = data.FirstOrDefault(t => t.Id == AssetsRes.Id);
            //    if (Assets != null)
            //    {
            //        AssetsRes.Products = Assets.AssetsProducts
            //            .Select(tp => new AssetsProductRes
            //            {
