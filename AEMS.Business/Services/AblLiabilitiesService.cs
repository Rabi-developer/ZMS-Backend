...
public interface IAblLiabilitiesService : IBaseService<AblLiabilitiesReq, AblLiabilitiesRes, AblLiabilities>
{
    Task<IList<AblLiabilities>> GetByParent(Guid ParentId);
}
public class AblLiabilitiesService : BaseService<AblLiabilitiesReq, AblLiabilitiesRes, AblLiabilitiesRepository, AblLiabilities>, IAblLiabilitiesService
{
    private readonly ApplicationDbContext _context;
    private readonly IAblLiabilitiesRepository _repository;
    protected readonly IUnitOfWork UnitOfWork;


    public AblLiabilitiesService(IUnitOfWork unitOfWork, ApplicationDbContext dbContext) : base(unitOfWork)
    {
        _context = dbContext;
        UnitOfWork = unitOfWork;
        _repository = UnitOfWork.GetRepository<AblLiabilitiesRepository>();

    }



    public async Task<IList<AblLiabilities>> GetByParent(Guid ParentId)
    {
        return await Repository.GetByParent(ParentId);
    }

    public async override Task<Response<Guid>> Add(AblLiabilitiesReq reqModel)
    {
        try
        {
            // Fetch the parent account if ParentAccountId is provided
            AblLiabilities parentAccount = null;
            if (reqModel.ParentAccountId.HasValue)
            {
                parentAccount = await _context.AblLiabilities
            ...
            else
            {
                ...
                var siblings = await _context.AblLiabilities
            ...
            var entity = reqModel.Adapt<AblLiabilities>();
            entity.Listid = listId;

            // Add the entity to the repository
            var ss = await Repository.Add((AblLiabilities)(entity as IMinBase ??
    ...
    public override async Task<Response<IList<AblLiabilitiesRes>>> GetAll(Pagination? paginate)
    {
        try
        {
            var pagination = paginate ?? new Pagination();

            // Include AblLiabilitiesProducts in the query
           ...
           .OrderBy(x => x.Id)
            ...
            if (!data.Any())
            {
                return new Response<IList<AblLiabilitiesRes>>
                {
                    Data = new List<AblLiabilitiesRes>(),
                    Misc = pag,
            ...
            var AblLiabilitiesResList = data.Adapt<List<AblLiabilitiesRes>>();

            //// Manually map AblLiabilitiesProducts to Products in the response
            //foreach (var AblLiabilitiesRes in AblLiabilitiesResList)
            //{
            //    var AblLiabilities = data.FirstOrDefault(t => t.Id == AblLiabilitiesRes.Id);
            //    if (AblLiabilities != null)
            //    {
            //        AblLiabilitiesRes.Products = AblLiabilities.AblLiabilitiesProducts
            //            .Select(tp => new AblLiabilitiesProductRes
            //            {
